...
**重做方向已定稿 → 见 [ADR 0034](0034-whatif-replay-batch.md)**（2026-08-08）。

采纳**异步 `ReplayBatch`** + **用户可选时间窗口（含全量）**：
窗口是显式口径而非隐形抽样，窗口内**全量**跑、**全部成功**才出数字，
任一条失败即整批拒答。同步形态（含本 ADR §3）已被否——
「一条脏数据全废」在大策略上几乎必然触发，用户永远拿不到结果。

★ 本 ADR §3.2「不落表」的理由在异步模型下**已被推翻**，
详见 0034 §3.2 的诚实说明。§2（能力已存在）、§3.1（关联键是同一条 input）、
§4（护栏）、§5.1（S0 实测）仍然有效并被 0034 继承。
