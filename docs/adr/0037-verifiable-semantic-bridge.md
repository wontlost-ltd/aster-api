...
| **4** | Stable IR Node IDs | 唯一全新的一件。★前置问题（是否真要做跨版本 change impact）**用户已拍板：要做**；ADR 0032 §6.1 同步修订 | 🟡 **Java 侧已完成**（`core#166`：复合键 `nodeId` + `contentHash`，5 变异验证 + 跨 JVM 确定性）；**TS 侧已完成**（`ts#173` + `test#141` 共享归一化，全语料 218/223 样本、99.3% 节点一致，见 §8.7）|
...
|---|---|
| 完全一致的样本 | **218 / 223** |
| 节点级一致率 | **8600 / 8661（99.3%）** |
| TS 解析失败 | 0 |

★余下 5 个（`eff_valid_all_caps` / `fetch_dashboard` / `interop_overload` /
`interop_sum` / `login`）**与 parity 门禁报告的已知 IR 分歧样本逐一对应**
（effects / interop 层），属**既有**技术债，非本次引入。
