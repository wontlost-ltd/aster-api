...
- ~~**不引入独立的节点 ID 体系**（如给每个 AST 节点发 UUID）。位置信息已经有了，再造一套是重复建设，且 UUID 跨编译不稳定~~
  **→ 2026-09-12 修订：本条的适用范围收窄，见 §6.1。**
- **不回填历史数据**（见 §4.3）
- **不把 `result` 放进 anchor 相关结构**。`TraceSkeleton` 的 PII 边界是**结构性保证**（没有 `result` 字段），本 ADR 不触碰这条线

### 6.1 关于「不引入独立节点 ID 体系」的修订（2026-09-12）

**用户已拍板：跨版本 change impact 要做**（ADR 0037 §4/§9 的核心能力）。
本条因此需要区分两种情况——原文把它们混为一谈了。

| | 本 ADR 的 `anchor` | ADR 0037 的 Stable Node ID |
|---|---|---|
| 形态 | `"L21C5-L21C22"`，**位置派生** | **结构派生**（命名作用域路径），非位置、非随机 |
| 稳定范围 | **同一策略版本内** | **跨版本** |
| 服务对象 | Phase 1 条件漏斗的跨执行聚合 | 原文 ↔ 程序双向导航、change impact |

**原文反对的是 UUID**（随机发号、跨编译不稳定）——这一点**依然有效**，
0037 的方案也不是 UUID。

**但原文的第二个理由「位置信息已经有了，再造一套是重复建设」只在同版本内成立。**
本 ADR §5 自己写明：anchor「策略源码改一行，下面所有 anchor 全变」，并**接受**
该代价（「跨策略版本的统计本就不该混算」）。即 anchor **在结构上就不解决**
跨版本身份问题——它不是「已经有了」，而是「有的是另一样东西」。

**修订后的表述**：

- ✅ 仍然不做：给节点发 **UUID** 或任何随机/编译序相关的 ID。
- ✅ 本 ADR 的 anchor 保持不变，继续服务同版本内的 trace 聚合。
- 🆕 允许引入**结构派生**的跨版本标识，归属 ADR 0037 步骤 4，**不替代 anchor**。

⚠️ **两者并存必须守住一条边界**：anchor 回答「同一次执行/同一版本里的哪个步骤」，
nodeId 回答「跨版本的哪个节点」。**不得用其中一个去做另一个的工作**——
混用会让「跨版本统计」悄悄退化成「同版本统计」，且不报错。
