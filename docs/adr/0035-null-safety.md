...
- 状态：**档位 A + C IMPLEMENTED / 档位 B 待排期**（2026-08-22）
  ...
  - 档位 B（编译期警告）：**判定为不必要，不做**（2026-08-23）——
    档位 C 落地后，B 原本要拦的四种情形已全部响亮失败，详见 §3 档位 B 小节
...
### ★ 结论：档位 B 判定为**不必要**（2026-08-23）

档位 C 落地后逐个实测 B 原本要拦的四种「未处理 null 就使用」：

| 情形 | 现状 |
|---|---|
| ① 参与算术 `Map.get(m,k) plus 1` | 运行期**报错** |
| ② 参与比较 `Map.get(m,k) at least 1` | 运行期**报错** |
| ③ 字段访问 `Map.get(m,k).x` | **编译期拒绝** |
| ④ 传给 builtin `List.length(Map.get(m,k))` | 运行期**报错** |

其中 ④ 起初**静默返回 0**——查证发现根因不止 null：五个 List builtin 对
**任何**非列表输入静默返回 `0/true/null/false/[x]`，而 Java 一律 throw
（既是双引擎分叉，也是静默错答案：`List.length(Some([1,2,3]))` 返回 0，
命中的非空列表被报成空）。已修（ts#120，改用既有的 `reqList`）。

**因此 B 的目标已由「C + 全链响亮失败」达成**，无需再做一遍数据流分析。
若将来希望把这些**运行期**错误进一步提前到**编译期**，那是独立的类型系统
增强议题（需要 `Maybe` 进入类型检查），届时另开 ADR，而非本档位 B。
